...
# =======================================================================
# GENERATORE DI TARGET CASUALI (RANDOM STEPS)
# =======================================================================
class RandomTargetGenerator:
    # NOVITA': generatore torch dedicato, indipendente dal seed globale fissato da
    # skrl (Runner chiama set_seed(cfg["seed"]) prima che questa classe venga
    # istanziata - senza un generatore separato, torch.rand qui produrrebbe sempre la
    # stessa sequenza ad ogni esecuzione dello script, dando l'impressione che i
    # riferimenti "casuali" siano sempre gli stessi).
    def __init__(self, num_envs, device, env_cfg, control_dt, hold_s, seed=None):
        ...
        # generatore RNG dedicato: se seed e' None, viene inizializzato da
        # torch.seed() (basato sull'orologio di sistema), quindi ogni esecuzione da'
        # target diversi; se vuoi riproducibilita', passa --seed con un valore fisso
...
def main(env_cfg, experiment_cfg: dict):
    ...
    # NOTA: Runner(...) fissa qui il seed globale di torch/numpy/random secondo
    # experiment_cfg["seed"] (es. 42 nel tuo agent.yaml) - e' per questo che il
    # RandomTargetGenerator, istanziato PIU' SOTTO, usa un torch.Generator dedicato
    # invece di affidarsi al RNG globale, altrimenti otterresti sempre la stessa
    # sequenza di target ad ogni esecuzione.
    ...
    # 4. Inizializzazione target e variabili grafici
    ...
    with torch.inference_mode():
        for step in range(args_cli.num_steps):
            # Forziamo il nostro target casuale
